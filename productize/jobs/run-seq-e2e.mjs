#!/usr/bin/env node
/**
 * Playwright E2E for minigame_master/core/demo/sequence_synthesis
 * Desktop: start → run → retreat
 * Mobile: start → run → force fail → force win
 * Writes:
 *   - minigame_master/capabilities/reports/runtime_e2e_sequence_synthesis_latest.json
 */
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/?theme=alchemy&recipeLength=3&seed=42&goalValue=6&durationSec=45`;
  const errors = [];
  const assertions = {};
  const observed = {};
  const flows = [];
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      const meta = await page.evaluate(() => window.__LW_SEQ_DEMO_META__ || null);
      observed.demoMeta = meta;
      assertions[`${prefix}_hasSpecHash`] =
        typeof meta?.specHash === "string" && meta.specHash.length > 0;
      assertions[`${prefix}_cardId`] = meta?.cardId === "sequence_synthesis";
      ...
      const skillProbe = await page.evaluate(() => {
        const adapter = window.__LW_SEQ_DEMO__;
        if (!adapter) return { ok: false };
        const nextId = adapter.state?.recipe?.[adapter.state?.stepIndex];
        const mat = adapter.state?.pool?.find((m) => m.id === nextId);
        if (mat) adapter.onMaterialClick?.(mat);
        return {
          ok: true,
          stepIndex: adapter.state?.stepIndex,
          progress: adapter.state?.progress,
          recipeLength: adapter.state?.recipe?.length
        };
      });
      observed.skillProbe = skillProbe;
      assertions[`${prefix}_skillInteraction`] =
        Boolean(skillProbe?.ok) &&
        ((skillProbe.stepIndex || 0) >= 1 || (skillProbe.progress || 0) > 0);
      assertions[`${prefix}_canvasNonblank`] = Boolean(runningCanvas.nonBlank);
      assertions[`${prefix}_specHash`] =
        duringStart.specHash === meta?.specHash && duringStart.cardId === "sequence_synthesis";
      ...
      if (vp.id === "mobile_720x1280") {
        ...
        await page.evaluate(() => {
          const adapter = window.__LW_SEQ_DEMO__;
          adapter?.forceComplete?.() || adapter?.finish?.(true, "objective_met");
        });
        await page.waitForFunction(
          () => {
            try {
      ...
      // Visual screenshot
      const shotDir = path.join(REPORTS_DIR, "visual/sequence_synthesis");
  ...
  const report = {
    ...
    gate: "sequence_synthesis_demo_e2e",
    status,
    createdAt: new Date().toISOString(),
    cardId: "sequence_synthesis",
    specHash: observed.demoMeta?.specHash || DEMO_SPEC_HASH_FALLBACK(),
    runtimeVersion: observed.demoMeta?.runtimeVersion || "minigame_master.core.demo.sequence_synthesis",
    releaseEligible,
    ...
    notes: [
      ...
      "skipBoss demo knobs for sequence synthesis puzzle."
    ],
    productionCertification: releaseEligible
      ? {
          ...
          waivers: [
            ...
            "standalone_export_host: demo browser E2E used as primary browser gate for sequence synthesis card"
          ]
        }
      : undefined,
  ...
  const outPath = path.join(REPORTS_DIR, "runtime_e2e_sequence_synthesis_latest.json");
  ...
  if (status === "passed") {
    const browserSummary = {
      ...
      releaseEligible,
      sourceReport: "runtime_e2e_sequence_synthesis_latest.json",
      assertions: report.assertions,
      errors: report.errors,
      flows: report.flows,
      productionCertification: report.productionCertification || null,
      notes: [
        "Derived from core demo Playwright E2E (sequence_synthesis).",
        "Per-card filename avoids clobbering survivor_horde standalone_browser_report.json."
      ]
    };
    fs.writeFileSync(
      path.join(REPORTS_DIR, "standalone_browser_report_sequence_synthesis.json"),
...
function DEMO_SPEC_HASH_FALLBACK() {
  return "sequence_synthesis:core_demo:v1_theme_skin";
}

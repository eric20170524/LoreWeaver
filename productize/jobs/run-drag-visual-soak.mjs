#!/usr/bin/env node
/**
 * Visual hygiene + soak for drag_collect_grid core demo.
 *
 * Env:
 ...
 *   visual_audit_drag_collect_grid_latest.json
 *   performance_report_drag_collect_grid_latest.json
 *   visual/drag_collect_grid/*.png
 */
...
const VISUAL_DIR = path.join(REPORTS_DIR, "visual/drag_collect_grid");
const CARD_PATH = path.join(LORE_ROOT, "minigame_master/gameplay/cards/drag_collect_grid.json");
const CARD_ID = "drag_collect_grid";
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/?theme=void`;
  const vite = spawn(
    VITE_BIN,
  ...
  try {
    ...
    if (!SKIP_SOAK) {
      ...
      const soakUrl = `http://127.0.0.1:${port}/?theme=void&durationSec=${soakDuration}&goalValue=9999&hazardRate=0.12`;
      ...
      while (Date.now() < endAt) {
        ...
        // keep sliding / tapping to exercise collect-dodge
        if (Math.random() > 0.35) {
          const x = 200 + Math.floor(Math.random() * 320);
          await page.mouse.move(x, 900);
          await page.mouse.down();
          await page.mouse.move(x + 40, 900, { steps: 4 });
          await page.mouse.up();
        }
  ...
  const visualReport = {
    ...
    gate: "visual_audit",
    target: "minigame_master/core/demo/drag_collect_grid",
    cardId: CARD_ID,
    status: visualStatus,

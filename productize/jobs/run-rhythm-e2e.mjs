#!/usr/bin/env node
/**
 * Playwright E2E for minigame_master/core/demo/rhythm_timing
 * Desktop: start → run → retreat
 * Mobile: start → run → force fail → force win
 * Writes:
 *   - minigame_master/capabilities/reports/runtime_e2e_rhythm_timing_latest.json
 */
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/?theme=temple&goalValue=6&durationSec=45`;
  const errors = [];
  const assertions = {};
  const observed = {};
  const flows = [];
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      const meta = await page.evaluate(() => window.__LW_RHYTHM_DEMO_META__ || null);
      observed.demoMeta = meta;
      assertions[`${prefix}_hasSpecHash`] =
        typeof meta?.specHash === "string" && meta.specHash.length > 0;
      assertions[`${prefix}_cardId`] = meta?.cardId === "rhythm_timing";
      ...
      assertions[`${prefix}_specHash`] =
        duringStart.specHash === meta?.specHash && duringStart.cardId === "rhythm_timing";
      ...
      if (vp.id === "mobile_720x1280") {
        ...
        const pauseProbe = await page.evaluate(async () => {
          const adapter = window.__LW_RHYTHM_DEMO__;
      ...
      // Visual screenshot
      const shotDir = path.join(REPORTS_DIR, "visual/rhythm_timing");
...
function DEMO_SPEC_HASH_FALLBACK() {
  return "rhythm_timing:core_demo:v1_theme_skin";
}

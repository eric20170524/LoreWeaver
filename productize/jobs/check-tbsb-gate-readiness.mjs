#!/usr/bin/env node
/**
 * Gate / production readiness for turn_based_skill_battle.
 *
...
const CARD_PATH = path.join(LORE_ROOT, "minigame_master/gameplay/cards/turn_based_skill_battle.json");
...
function main() {
  ...
  const e2e = readJson(path.join(REPORTS, "runtime_e2e_turn_based_skill_battle_latest.json"));
  const browser = readJson(path.join(REPORTS, "standalone_browser_report_turn_based_skill_battle.json"));
  const visual = readJson(path.join(REPORTS, "visual_audit_turn_based_skill_battle_latest.json"));
  const perf = readJson(path.join(REPORTS, "performance_report_turn_based_skill_battle_latest.json"));
  const recipe = readJson(path.join(REPORTS, "level_recipe_compile_tbsb_latest.json"));

  const checks = [
    check("card_present", Boolean(card?.id === "turn_based_skill_battle"), card?.id),
    check(
    ...
    check("e2e_cardId", e2e?.cardId === "turn_based_skill_battle", e2e?.cardId),
    check(
      "theme_fixtures",
      fs.existsSync(
        path.join(
          LORE_ROOT,
          "minigame_master/gameplay/cards/fixtures/turn_based_skill_battle/theme_content_pack.fixture.json"
        )
      ),
      "temple"
    ),
    check(
      "demo_present",
      fs.existsSync(path.join(LORE_ROOT, "minigame_master/core/demo/turn_based_skill_battle/main.js")),
      "demo/main.js"
    )
  ];
  ...
  const report = {
    schemaVersion: "loreweaver.tbsb-gate-readiness.v1",
  ...
  fs.writeFileSync(
    path.join(REPORTS, "tbsb_gate_readiness_latest.json"),

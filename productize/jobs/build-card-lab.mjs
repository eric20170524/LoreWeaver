...
export async function buildCardLab() {
  ...
  await build({absWorkingDir:ROOT,entryPoints:['productize/card-lab/iframe-node.ts'],bundle:true,platform:'browser',format:'iife',minify:true,outfile:path.join(OUT,'iframe-node.js'),logLevel:'warning'});
  fs.mkdirSync(path.join(OUT,'nodes'),{recursive:true});
  fs.copyFileSync(path.join(ROOT,'productize/card-lab/iframe-node.html'),path.join(OUT,'nodes/node1.html'));
  ...
  const files = ['nodes/node1.html', 'iframe-node.js', 'index.html', 'game.js', 'game.js.LEGAL.txt', 'LICENSE', 'THIRD_PARTY_NOTICES.txt'].filter(name => fs.existsSync(path.join(OUT, name)));
  const manifest = { revision, cards: ['dodge_counter_boss', 'rhythm_timing', 'drag_collect_grid', 'sequence_synthesis', 'turn_based_skill_battle', 'side_scrolling_brawler', 'survivor_horde', 'reaction_pick', 'energy_balance', 'observe_capture', 'drag_to_core', 'pressure_survival', 'rune_connect_sequence', 'branching_dialogue_check', 'shooter_duel', 'maze_exploration_choice', 'platform_escape', 'hazard_collect_waves', 'sequence_puzzle_combo', 'rhythm_then_pickup', 'qix_area_capture', 'point_drag_progression', 'node_iframe_microgame'], releaseEligible: false,

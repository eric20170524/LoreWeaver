#!/usr/bin/env node
/**
 * Visual hygiene + soak for rhythm_timing core demo.
 *
 * Env:
 ...
 *   visual_audit_rhythm_timing_latest.json
 *   performance_report_rhythm_timing_latest.json
 *   visual/rhythm_timing/*.png
 */
...
const DEMO_ROOT = path.join(LORE_ROOT, "minigame_master/core/demo/rhythm_timing");
...
const VISUAL_DIR = path.join(REPORTS_DIR, "visual/rhythm_timing");
const CARD_PATH = path.join(LORE_ROOT, "minigame_master/gameplay/cards/rhythm_timing.json");
const CARD_ID = "rhythm_timing";
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/?theme=temple`;
  const vite = spawn(
    VITE_BIN,
  ...
  try {
    ...
    if (!SKIP_SOAK) {
      ...
      const soakUrl = `http://127.0.0.1:${port}/?theme=temple&durationSec=${soakDuration}&goalValue=9999`;
      ...
      while (Date.now() < endAt) {
        ...
        // keep tapping to exercise interaction
        if (Math.random() > 0.4) {
        ...
        const stillRunning = await page.evaluate(() => {
          try {
            ...
            return s.status === "running" || s.mode === "run";
          } catch {
            return false;
          }
        });
        if (!stillRunning) {
          // restart if ended early
          const hasStart = await page.locator('[data-testid="start-run"]').count();
          const hasBack = await page.locator('[data-testid="back-menu"]').count();
          if (hasBack) {
            await page.locator('[data-testid="back-menu"]').click().catch(() => {});
            await page.waitForTimeout(300);
          }
          if ((await page.locator('[data-testid="start-run"]').count()) > 0) {
            await page.locator('[data-testid="start-run"]').click();
  ...
  const visualReport = {
    ...
    gate: "visual_audit",
    target: "minigame_master/core/demo/rhythm_timing",
    cardId: CARD_ID,
    status: visualStatus,

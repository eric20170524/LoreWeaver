#!/usr/bin/env node
/**
 * Visual hygiene + soak for turn_based_skill_battle core demo.
 *
 * Env:
 ...
 *   visual_audit_turn_based_skill_battle_latest.json
 *   performance_report_turn_based_skill_battle_latest.json
 *   visual/turn_based_skill_battle/*.png
 */
...
const VISUAL_DIR = path.join(REPORTS_DIR, "visual/turn_based_skill_battle");
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/?theme=sect`;
  const vite = spawn(
    VITE_BIN,
  ...
  try {
    ...
    if (!SKIP_SOAK) {
      ...
      const soakUrl = `http://127.0.0.1:${port}/?theme=sect&durationSec=${soakDuration}&enemyHp=99999&enemyAtk=0`;
      ...
      while (Date.now() < endAt) {
        ...
        if (Math.random() > 0.85) {
          await page.evaluate(() => {
            const a = window.__LW_TBSB_DEMO__;
            const skill = a?.config?.skillDeck?.[0];
            if (a && skill && a.state?.turn === "player") a.onSkillClick?.(skill);
          });
        }
  ...
  const visualReport = {
    ...
    gate: "visual_audit",
    target: "minigame_master/core/demo/turn_based_skill_battle",
    cardId: CARD_ID,
    status: visualStatus,

#!/usr/bin/env node
/**
 * Gate / production readiness for drag_collect_grid.
 *
...
const CARD_PATH = path.join(LORE_ROOT, "minigame_master/gameplay/cards/drag_collect_grid.json");
...
function main() {
  ...
  const e2e = readJson(path.join(REPORTS, "runtime_e2e_drag_collect_grid_latest.json"));
  const browser = readJson(path.join(REPORTS, "standalone_browser_report_drag_collect_grid.json"));
  const visual = readJson(path.join(REPORTS, "visual_audit_drag_collect_grid_latest.json"));
  const perf = readJson(path.join(REPORTS, "performance_report_drag_collect_grid_latest.json"));
  const recipe = readJson(path.join(REPORTS, "level_recipe_compile_drag_latest.json"));

  const checks = [
    check("card_present", Boolean(card?.id === "drag_collect_grid"), card?.id),
    check(
    ...
    check("e2e_cardId", e2e?.cardId === "drag_collect_grid", e2e?.cardId),
    check(
      "theme_fixtures",
      fs.existsSync(
        path.join(
          LORE_ROOT,
          "minigame_master/gameplay/cards/fixtures/drag_collect_grid/theme_content_pack.fixture.json"
        )
      ),
      "temple"
    ),
    check(
      "demo_present",
      fs.existsSync(path.join(LORE_ROOT, "minigame_master/core/demo/drag_collect_grid/main.js")),
      "demo/main.js"
    )
  ];
  ...
  const report = {
    schemaVersion: "loreweaver.drag-gate-readiness.v1",
  ...
  fs.writeFileSync(
    path.join(REPORTS, "drag_gate_readiness_latest.json"),

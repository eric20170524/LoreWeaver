#!/usr/bin/env node
/**
 * Gate / production readiness for sequence_synthesis.
 *
...
function main() {
  ...
  const e2e = readJson(path.join(REPORTS, "runtime_e2e_sequence_synthesis_latest.json"));
  const browser = readJson(path.join(REPORTS, "standalone_browser_report_sequence_synthesis.json"));
  const visual = readJson(path.join(REPORTS, "visual_audit_sequence_synthesis_latest.json"));
  const perf = readJson(path.join(REPORTS, "performance_report_sequence_synthesis_latest.json"));
  const recipe = readJson(path.join(REPORTS, "level_recipe_compile_seq_latest.json"));

  const checks = [
    check("card_present", Boolean(card?.id === "sequence_synthesis"), card?.id),
    check(
    ...
    check("e2e_cardId", e2e?.cardId === "sequence_synthesis", e2e?.cardId),
    check(
      "theme_fixtures",
      fs.existsSync(
        path.join(
          LORE_ROOT,
          "minigame_master/gameplay/cards/fixtures/sequence_synthesis/theme_content_pack.fixture.json"
        )
      ),
      "temple"
    ),
    check(
      "demo_present",
      fs.existsSync(path.join(LORE_ROOT, "minigame_master/core/demo/sequence_synthesis/main.js")),
      "demo/main.js"
    )
  ];
  ...
  const report = {
    schemaVersion: "loreweaver.seq-gate-readiness.v1",
  ...
  fs.writeFileSync(
    path.join(REPORTS, "seq_gate_readiness_latest.json"),

...
export function WorkspaceSelector({ activeWorkspaceId, onSelectWorkspace }: WorkspaceSelectorProps) {
    ...
    <div className="relative">
      ...
      >
        ...
        <span className="truncate max-w-[120px]">
          {activeWorkspace ? activeWorkspace.name : "未选择工作区"}
        </span>
      </button>
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-3 overflow-y-auto min-h-[100px] flex-1">
              {workspaces.length === 0 ? (
                <div className="text-center text-slate-500 text-xs py-4">无历史项目</div>
              ) : (
                <div className="space-y-2">
                  {workspaces.map((ws) => (
                    ...
                    >
                      ...
                      <p className="text-3xs text-slate-500 truncate">主题: {ws.theme}</p>
                      <p className="text-3xs text-slate-600 mt-1">
                        修改于 {new Date(ws.lastModifiedAt).toLocaleString()}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="p-3 border-t border-slate-900 bg-slate-900/30 flex flex-col gap-2">
              <span className="text-3xs text-slate-500 uppercase tracking-wider font-semibold mb-1">新建隔离工作区</span>
              <input 
                value={newName} onChange={e => setNewName(e.target.value)}
                placeholder="工作区标识 (e.g. 遮天同人)" 
              ...
              <input 
                ...
                placeholder="设定主轴 (e.g. 九龙拉棺，星空古路)" 
                className="bg-slate-950 border border-slate-800 rounded px-2 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-emerald-500"
              />
              <button 

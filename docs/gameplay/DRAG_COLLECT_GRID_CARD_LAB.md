...
3918000 的独立收集浏览器检查5/7通过，默认通关和1颗目标两项失败：PlayabilityContract 的历史启发式会把16/40秒改为13，把1改为8。游戏确实能通关，但不是用户设置的目标，因此不能算通过。

修复共享参数边界，对 drag_collect_grid / collect_dodge 保留合法的1～200目标，不再按时限静默改难度；不合理配额仅警告。其余旧卡迁移策略未扩展改动。补充3项宿主规范化、重复规范化和真实宿主→适配器参数回归，新收集测试19项，合计67项。浏览器仍要求默认16颗和一颗目标，未放宽断言。


同次独立任务的闪避6/6通过；节奏5/8通过。节奏失败报告显示已发出的输入都按实际偏差正确判定，但驱动多次错过仅30ms宽的观测区间，整拍无输入、连击中断。驱动改用较宽的只读观测区间并根据已记录的真实输入偏差估计外部投递延迟；关闭连续trace截图编码，保留显式截图与操作轨迹。游戏判定窗口、伤害、目标和连击条件均未修改，仍要求正常输入取得胜利和真实Good/Perfect。

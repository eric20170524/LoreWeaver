...
function read(relPath) {
  return fs.readFileSync(path.join(loreRoot, relPath), "utf8");
}

function collectFiles(dirRel, suffix = ".js") {
  const dir = path.join(loreRoot, dirRel);
...
const runnerSource = read("src/game/GameRunner.ts");
checks.push({
...
checks.push({
  ...
  status: /adapter[\s\S]{0,120}retreat\s*===\s*["']function["'][\s\S]{0,160}\.retreat\(\)/.test(runnerSource)
    ? "passed"
    : "failed",
  message: "GameRunner LevelActiveScene retreat button triggers adapter.retreat() when adapter is active"
});

for (const file of targetFiles.filter((item) => /\/modifiers\/[^/]+Modifier\.js$/.test(item))) {
  const source = read(file);
  const ownsAsyncResources = /time\.(?:addEvent|delayedCall)\s*\(|events?\.on\s*\(|track(?:Timer|Listener|DomListener)\s*\(/.test(source);
  const hasCustomUninstall = /uninstall\(\w*\)/.test(source);
  checks.push({
    id: `${file}:modifier_uninstall`,
    status: hasCustomUninstall || !ownsAsyncResources ? "passed" : "failed",
    message: ownsAsyncResources
      ? "Resource-owning modifier exposes a custom uninstall cleanup hook"
      : "Modifier owns no async listener/timer resource requiring custom cleanup"
  });
}
